...
daemon 暫時不可用時，Adapter SDK fail-open，Hermes 繼續使用原生功能；只有安全且明確的 capture 會進 outbox，`beforeTask` 讀取完成後會背景重送，下一次 `beforeTask` 或 `onUserMessage` 仍會再次重試。完整 transcript、assistant 推理、token 與秘密不會寫入 outbox、Brain 或 backup。
