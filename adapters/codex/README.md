...
適合在你需要把個人偏好與多個專案的決策集中管理，並在 Codex、Hermes、Claude Code 或 OpenClaw 之間切換時仍保持一致。v0.3 建議只設定 workspace，讓 daemon 自動選 Personal／Project；公司、團隊或產品直接作為 Project。

## 需要什麼

- 已在本機建置 Memlume Core（包含 `@memlume/adapter-sdk` 與 MCP Server）。
- 已建立 Codex 的 agent installation；v0.3 的 `setup adapter` 會自動掛載 Personal 與 workspace Project 的權限。
...
| `UserPromptSubmit` | 以 `beforeTask` 讀取已掛載的 **Primary Project → 命中的 Linked Project → 相關 Personal** Context，再以暫時 `additionalContext` 注入本回合；同時以 `onUserMessage` 將使用者訊息交給 Core。 |
...
- 只讀取受限的 Project Brain，絕不回退到 Personal 或其他未匹配 Project。
