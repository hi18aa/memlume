...
| `UserPromptSubmit` | 先讀取 Project Brain 的 Shared Context，再以暫時 `additionalContext` 注入本回合；明確記憶請求會交給獨立的本機寫入工作交由 Core 處理。 |
| `Stop` | 以獨立的本機寫入工作記錄本回合最後的 assistant 輸出作為 task audit，hook 永遠回傳 `{}`；它不是 session 結束事件。 |
...
若 daemon 暫時不可用，hook 仍會成功結束，不會阻斷 Codex。寫入工作不會等待既有 outbox lock 或 backlog；它會在 hook 回傳後繼續完成。只有明確記憶請求能進入本機 outbox 等待下次 `UserPromptSubmit` 重送；`Stop` 的 audit 不會被離線宣稱為已保存，也不會假裝 session-end 重送。
