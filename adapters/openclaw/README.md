...
adapter token 只放在啟動 OpenClaw Gateway 的環境變數，絕不放入 `openclaw.plugin.json`、OpenClaw 設定檔或 Git：
...
`MEMLUME_HOME` 必須是完整 Memlume repository；Plugin 會確認實際載入的 SDK 仍在此目錄內。`MEMLUME_DAEMON_URL`（預設 `http://127.0.0.1:3849`）與 `MEMLUME_OUTBOX_DIRECTORY` 可選填。
