...
| `onUserMessage(...)` | 唯一的自動 capture 入口。只有明確記憶要求交給 Core；一般訊息可被忽略。主 Agent 寫入採明確 Brain → `defaultWriteBrainId`（Project Brain）→ `rejected`，永不回退 Personal。 |
...
| `queued` | Daemon 暫時不可用，且 SDK 已安全寫入本機 outbox；待下一次 `beforeTask` 或 `onUserMessage` 重送。 |
